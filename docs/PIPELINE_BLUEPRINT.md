...
*   **[METHODOLOGICAL FLAG] Why 10,000 total designs?**
    *   *Reasoning:* Protein design is a numbers game. Most AI-generated designs will fail in the real world due to aggregation, misfolding, or lack of expression. Generating 10,000 gives us a statistically significant pool to filter down to the top ~10-50 candidates for actual lab testing.
    *   *Citation:* Watson, J. L., et al. (2023). De novo design of protein structure and function with RFdiffusion. *Nature*, 620(7976), 1089-1100.
...
*   **[METHODOLOGICAL FLAG] Why generate 8 sequences per backbone?**
    *   *Reasoning:* Biology is flexible. Multiple different sequences can fold into the same shape. By asking ProteinMPNN to generate 8 options per backbone, we increase our chances of finding a highly stable sequence. This is a standard empirical practice in the field to sample the sequence space adequately.
...
*   **Output:** 1,000 `.fa` (FASTA) files. **Note:** Each of these 1,000 files contains 8 different sequences inside it. So, we have 1,000 files, but 8,000 total sequences.

### Step C: Sequence Selection (Python Script)
*   **Action:** A custom Python script reads the ProteinMPNN outputs. For each backbone, it looks at the 8 generated sequences inside the `.fa` file, picks the single best one based on the "MPNN score" (lowest energy/highest confidence), and discards the other 7.
*   **[METHODOLOGICAL FLAG] Why pick only 1 sequence out of 8?**
    *   *Reasoning:* The next step (ColabFold) is extremely computationally expensive. Folding all 8 sequences for 1,000 backbones (8,000 folds) would take weeks. By picking the single best sequence per backbone based on the MPNN score, we save massive amounts of time and compute power while retaining the highest quality candidates.
*   **Output:** A single, clean `.fa` file containing exactly 1,000 optimized sequences (one for each original backbone).
...
*   **[METHODOLOGICAL FLAG] Why does ColabFold output 5 models per sequence?**
    *   *Reasoning:* This is hardcoded into AlphaFold2's architecture. DeepMind trained 5 slightly different neural networks to prevent bias and capture structural ensembles. ColabFold runs your sequence through all 5 networks. If all 5 predict the exact same shape, you have high confidence it will work in reality.
...
*   **Action:** A script reads the ColabFold `.json` files and filters the designs based on three critical metrics. These thresholds are standard in the field for identifying successful *de novo* binders.
*   **[METHODOLOGICAL FLAG] Why pLDDT > 80?**
    *   *Reasoning:* pLDDT (predicted Local Distance Difference Test) measures confidence in the overall fold. A score > 80 indicates high confidence that the monomer will fold into a stable, well-defined structure.
    *   *Citation:* Jumper, J., et al. (2021). *Nature*.
*   **[METHODOLOGICAL FLAG] Why iPAE < 10?**
    *   *Reasoning:* iPAE (interface Predicted Aligned Error) measures confidence that the binder will stick to the target. An iPAE < 10 Å is the empirical threshold established by the Baker Lab for predicting successful protein-protein interactions.
    *   *Citation:* Evans, R., et al. (2021). Protein complex prediction with AlphaFold-Multimer. *bioRxiv*.
*   **[METHODOLOGICAL FLAG] Why RMSD < 2.0 Å?**
    *   *Reasoning:* RMSD (Root Mean Square Deviation) measures how closely the ColabFold prediction matches the original RFdiffusion backbone. An RMSD < 2.0 Å is the standard structural biology threshold for considering two folds to be "identical."
    *   *Citation:* Watson, J. L., et al. (2023). *Nature*.
...
│   ├── 2_proteinmpnn_out/      # 10 MB of FASTA files (1,000 files, 8,000 sequences)
